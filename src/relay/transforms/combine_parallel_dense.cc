 ...
 protected:
  ...
  void UpdateGroupOutput(const Expr& data, const Group& branches, size_t depth,
    ...
    for (const auto& branch : branches) {
      ...
      int64_t out_dims = *tir::as_const_int(weight->shape[0]);
      std::vector<int64_t> begin = {0, index};
      std::vector<int64_t> end = {-1, out_dims};
      index += out_dims;
 ...
 private:
  std::tuple<Expr, IndexExpr> TransformWeight(const Group& branches) {
    ...
    for (const auto& branch : branches) {
      ...
      output_dims += *tir::as_const_int(weight->type_as<TensorTypeNode>()->shape[0]);
    }
    return std::make_tuple(MakeConcatenate(Tuple(weights), 0),
                           tir::make_const(DataType::Int(32), output_dims));
  }
};
